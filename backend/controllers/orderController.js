...
exports.deleteOrderByUser = asyncErrosFunction(async (req, res, next) => {
  const order = await orderSchema.findById(req.params.id, req.user._id);
  ...
  if (order.orderStatus !== "Delivered") {
    ...
    res.status(404).json({
      success: false,
      message: "You don't have permission to delete delivered orders",
    });
  }
